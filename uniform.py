...
# Generate the simulated data from the uniform prior.
edge_prob = 0.5
G_simul = np.empty((n_p, n_rep), dtype=object)
data_simul = [np.empty((n_rep, 2 * p, p)) for p in p_seq]
rng = np.random.Generator(np.random.SFC64(seed=0))
igraph.set_random_number_generator(random.Random(wwa.random_seed(rng)))

for p_ind in range(n_p):
    p = p_seq[p_ind]

    for r in range(n_rep):
        G_true = igraph.Graph.Erdos_Renyi(n=p, p=edge_prob, directed=False)
        G_simul[p_ind, r] = G_true

        data_simul[p_ind][r, :, :] = rng.multivariate_normal(
            mean=np.zeros(p),
            cov=np.linalg.inv(wwa.rgwish_identity(G=G_true, df=3.0, rng=rng)),
...
for p_ind in range(n_p):
    ...
    def MCMC(r, rng, delayed_accept=True, loc_bal=True, DCBF=False):
        return wwa.MCMC(
            G_init=G_simul[p_ind, r], n_iter=11000,
            data=data_simul[p_ind][r, :, :], edge_prob=edge_prob, rng=rng,
            verbose=False, delayed_accept=delayed_accept, loc_bal=loc_bal,
            DCBF=DCBF
        )
